...
import routesetMappingService from '../utils/prosbc/routesetMappingService.js';
import proSbcInstanceService from '../services/proSbcInstanceService.js';

const router = express.Router();

// In-memory job store for background generate-all jobs
const generateAllJobs = new Map();
const activateAllJobs = new Map();
const validateAllJobs = new Map();
...
// Generate routing database on all registered ProSBC instances
router.post('/generate-routing-all', async (req, res) => {
  try {
    // Optional systemId can be provided in body or query
    const systemId = req.body.systemId || req.query.systemId || '1';
  const background = req.body.background === true || req.body.background === 'true' || req.query.background === 'true' || false;

    // Get all active instances
    const instances = await proSbcInstanceService.getAllInstances();
    // Run generation in parallel with a configurable concurrency limit
    const results = [];
    const concurrency = Math.max(1, parseInt(req.body.concurrency || req.query.concurrency || process.env.GENERATE_ALL_CONCURRENCY || '5', 10));

    console.log(`[GenerateAll] Running generate on ${instances.length} instances with concurrency=${concurrency} (background=${background})`);

    const runGenerator = async (jobContext) => {
      let idx = 0;
      let active = 0;

      await new Promise((resolve) => {
        const next = () => {
          if (idx >= instances.length && active === 0) return resolve();

          while (active < concurrency && idx < instances.length) {
            ...
            (async (inst) => {
              const instanceId = inst.id;
              try {
                console.log(`[GenerateAll] Generating routing DB for instance: ${instanceId}`);
                const genResult = await routesetMappingService.generateRoutingDatabase(systemId, instanceId);
                const entry = { instance: instanceId, url: inst.baseUrl || inst.baseURL || inst.base_url || null, success: !!genResult.success, message: genResult.message || null, details: genResult };
                results.push(entry);
                if (jobContext) {
                  jobContext.results.push(entry);
                }
              } catch (err) {
                console.error(`[GenerateAll] Instance ${inst.id} error:`, err.message || err);
                const entry = { instance: inst.id, url: inst.baseUrl || inst.baseURL || inst.base_url || null, success: false, error: err.message || String(err) };
                results.push(entry);
                if (jobContext) {
                  jobContext.results.push(entry);
                }
    ...
    if (background) {
      ...
      const jobContext = { id: jobId, createdAt: Date.now(), status: 'running', results: [], concurrency, systemId };
      generateAllJobs.set(jobId, jobContext);

      // run asynchronously
      (async () => {
        try {
          await runGenerator(jobContext);
          jobContext.status = 'completed';
          jobContext.completedAt = Date.now();
        } catch (err) {
          jobContext.status = 'failed';
          jobContext.error = err.message || String(err);
          jobContext.completedAt = Date.now();
        }
      })();

      return res.json({ success: true, jobId, message: 'Generation started in background' });
    } else {
      // run in-request and wait for completion
...
// Get status for a background generate job
router.get('/generate-routing-all/status/:jobId', async (req, res) => {
  try {
    ...
    console.error('[GenerateAll Status] Error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// Activate configuration on all instances (supports background)
router.post('/activate-configuration-all', async (req, res) => {
  try {
    const configurationId = req.body.configurationId || req.query.configurationId;
    const systemId = req.body.systemId || req.query.systemId || '1';
    const background = req.body.background === true || req.body.background === 'true' || req.query.background === 'true' || false;
    const concurrency = Math.max(1, parseInt(req.body.concurrency || req.query.concurrency || process.env.GENERATE_ALL_CONCURRENCY || '5', 10));

    if (!configurationId) return res.status(400).json({ success: false, error: 'Missing configurationId' });

    const instances = await proSbcInstanceService.getAllInstances();
    const results = [];

    const runActivator = async (jobContext) => {
        ...
        const next = () => {
          ...
          while (active < concurrency && idx < instances.length) {
            ...
            (async (inst) => {
              try {
                const instanceId = inst.id;
                console.log(`[ActivateAll] Activating config ${configurationId} on instance ${instanceId}`);
                const result = await routesetMappingService.activateConfiguration(configurationId, systemId, instanceId);
                const entry = { instance: instanceId, url: inst.baseUrl || inst.baseURL || inst.base_url || null, success: !!result.success, message: result.message || null, details: result };
                results.push(entry);
                if (jobContext) jobContext.results.push(entry);
              } catch (err) {
                console.error('[ActivateAll] Error on instance', inst.id, err.message || err);
                const entry = { instance: inst.id, url: inst.baseUrl || inst.baseURL || inst.base_url || null, success: false, error: err.message || String(err) };
                results.push(entry);
                if (jobContext) jobContext.results.push(entry);
              } finally {
    ...
    if (background) {
      const jobId = createJobId();
      const jobContext = { id: jobId, createdAt: Date.now(), status: 'running', results: [], configurationId, systemId, concurrency };
      activateAllJobs.set(jobId, jobContext);
      (async () => {
        try {
          await runActivator(jobContext);
          jobContext.status = 'completed'; jobContext.completedAt = Date.now();
        } catch (err) {
          jobContext.status = 'failed'; jobContext.error = err.message || String(err); jobContext.completedAt = Date.now();
        }
      })();
      return res.json({ success: true, jobId, message: 'Activation started in background' });
    } else {
      await runActivator(null);
      return res.json({ success: true, results });
    }
  } catch (err) {
...
router.get('/activate-configuration-all/status/:jobId', async (req, res) => {
  try {
    const jobId = req.params.jobId;
    if (!jobId) return res.status(400).json({ success: false, error: 'Missing jobId' });
    const job = activateAllJobs.get(jobId);
    if (!job) return res.status(404).json({ success: false, error: 'Job not found' });
    return res.json({ success: true, job });
  } catch (err) {
    console.error('[ActivateAll Status] Error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// Validate configuration on all instances (supports background)
router.post('/validate-configuration-all', async (req, res) => {
  try {
    const configurationId = req.body.configurationId || req.query.configurationId;
    const systemId = req.body.systemId || req.query.systemId || '1';
    const background = req.body.background === true || req.body.background === 'true' || req.query.background === 'true' || false;
    ...
    const runValidator = async (jobContext) => {
      let idx = 0; let active = 0;
      await new Promise((resolve) => {
        const next = () => {
          if (idx >= instances.length && active === 0) return resolve();
          while (active < concurrency && idx < instances.length) {
            ...
            (async (inst) => {
              try {
                const instanceId = inst.id;
                console.log(`[ValidateAll] Validating config ${configurationId} on instance ${instanceId}`);
                const result = await routesetMappingService.validateConfiguration(configurationId, systemId, instanceId);
                const entry = { instance: instanceId, url: inst.baseUrl || inst.baseURL || inst.base_url || null, success: !!result.success, message: result.message || null, details: result };
                results.push(entry);
                if (jobContext) jobContext.results.push(entry);
              } catch (err) {
                console.error('[ValidateAll] Error on instance', inst.id, err.message || err);
                const entry = { instance: inst.id, url: inst.baseUrl || inst.baseURL || inst.base_url || null, success: false, error: err.message || String(err) };
                results.push(entry);
                if (jobContext) jobContext.results.push(entry);
              } finally {
    ...
    if (background) {
      const jobId = createJobId();
      const jobContext = { id: jobId, createdAt: Date.now(), status: 'running', results: [], configurationId, systemId, concurrency };
      validateAllJobs.set(jobId, jobContext);
      (async () => {
        try {
          await runValidator(jobContext);
          jobContext.status = 'completed'; jobContext.completedAt = Date.now();
        } catch (err) {
          jobContext.status = 'failed'; jobContext.error = err.message || String(err); jobContext.completedAt = Date.now();
        }
      })();
      return res.json({ success: true, jobId, message: 'Validation started in background' });
    } else {
      await runValidator(null);
      return res.json({ success: true, results });
    }
  } catch (err) {
...
router.get('/validate-configuration-all/status/:jobId', async (req, res) => {
  try {
    const jobId = req.params.jobId;
    if (!jobId) return res.status(400).json({ success: false, error: 'Missing jobId' });
    const job = validateAllJobs.get(jobId);
    if (!job) return res.status(404).json({ success: false, error: 'Job not found' });
    return res.json({ success: true, job });
  } catch (err) {
    console.error('[ValidateAll Status] Error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
...
// Debug: activate configuration on a single instance and verify active config
router.post('/debug/activate-instance', async (req, res) => {
  try {
    const { instanceId, configurationId, systemId = '1' } = req.body;
    if (!instanceId || !configurationId) return res.status(400).json({ success: false, error: 'Missing instanceId or configurationId' });

    console.log(`[Debug Activate] Starting for instance=${instanceId}, configuration=${configurationId}`);

    const activateResult = await routesetMappingService.activateConfiguration(configurationId, systemId, instanceId).catch(err => ({ success: false, error: err.message || String(err) }));

    // After activation attempt, fetch current available configurations to verify
    let configs = null;
    try {
      configs = await routesetMappingService.getAvailableConfigurations(systemId, instanceId);
    } catch (cfgErr) {
      console.warn('[Debug Activate] Failed to fetch configurations after activation:', cfgErr.message || cfgErr);
    }

    return res.json({ success: true, attempt: { instanceId, configurationId, systemId }, activateResult, configurations: configs });
  } catch (err) {
...
// Debug: validate configuration on a single instance and return raw result
router.post('/debug/validate-instance', async (req, res) => {
  try {
    const { instanceId, configurationId, systemId = '1' } = req.body;
    if (!instanceId || !configurationId) return res.status(400).json({ success: false, error: 'Missing instanceId or configurationId' });

    console.log(`[Debug Validate] Starting for instance=${instanceId}, configuration=${configurationId}`);

    const validateResult = await routesetMappingService.validateConfiguration(configurationId, systemId, instanceId).catch(err => ({ success: false, error: err.message || String(err) }));

    return res.json({ success: true, attempt: { instanceId, configurationId, systemId }, validateResult });
  } catch (err) {
    console.error('[Debug Validate] Error:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
