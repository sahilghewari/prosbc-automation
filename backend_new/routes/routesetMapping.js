...
router.post('/generate-database', async (req, res) => {
  try {
    const result = await routesetService.generateRoutingDatabase();
    res.json(result);
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get('/configurations', async (req, res) => {
  try {
    const configs = await routesetService.getAvailableConfigurations();
...
router.post('/configurations/:id/activate', async (req, res) => {
  try {
    const result = await routesetService.activateConfiguration(req.params.id);
    res.json(result);
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
